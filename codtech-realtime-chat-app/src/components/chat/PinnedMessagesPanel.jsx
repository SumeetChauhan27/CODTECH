...
export default function PinnedMessagesPanel({ roomData, onClose }) {
  ...
  return (
    <div className="absolute right-0 top-0 bottom-0 w-80 bg-white border-l border-zinc-200/60 shadow-xl z-30 flex flex-col animate-in slide-in-from-right-8 duration-300">
      <div className="flex items-center justify-between px-6 py-5 border-b border-zinc-100 shrink-0">
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 bg-orange-100 rounded-lg flex items-center justify-center">
            <Pin className="w-4 h-4 text-orange-500" />
          </div>
          <h3 className="font-bold text-zinc-900 tracking-tight">Pinned Messages</h3>
        </div>
        <button 
          onClick={onClose} 
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-zinc-50/50">
        {loading ? (
          ...
          <div className="text-center py-8">
            ...
            <p className="text-zinc-500 font-medium text-sm">No pinned messages in this room yet.</p>
          </div>
        ) : (
          pinnedMessages.map((msg) => (
            <div key={msg.id} className="bg-white p-4 rounded-2xl shadow-sm border border-zinc-100 flex flex-col gap-2">
              ...
              {msg.imageURL && (
                <div className="mt-2 rounded-xl overflow-hidden border border-zinc-100">
